"""Centralized logging setup for Harness Agent.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.hermes/logs/`` (profile-aware via ``get_harness_home()``).
...
Component separation:
    ...
    gui.log receives dashboard-side records from ``harness_cli.web_server``,
    ``harness_cli.pty_bridge``, ``tui_gateway.*``, and ``uvicorn.*``.
...
from harness_constants import get_config_path, get_harness_home
...
COMPONENT_PREFIXES = {
    ...
    "cron": ("cron",),
    "gui": (
        "harness_cli.web_server",
        "harness_cli.pty_bridge",
        "tui_gateway",
        "uvicorn",
    ),
}
...
def setup_logging(
    ...
    *force* is ``True``.
    ...
    hermes_home
        ...
        ``get_harness_home()`` (profile-aware).
    log_level
    ...
    home = hermes_home or get_harness_home()
    log_dir = home / "logs"
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    def __init__(self, *args, **kwargs):
        from harness_cli.config import is_managed
