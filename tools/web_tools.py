...
def _get_backend() -> str:
    """Name of the active web backend, resolved via the registry.

    Single source of truth: delegates to
    :func:`agent.web_search_registry.get_active_search_provider` (explicit
    config wins → single-eligible → availability-filtered legacy walk) and
    returns its name, or ``""`` when none is available. This replaces the
    former parallel env-auto-detect cascade so there is exactly one selection
    path. Used by the ``__main__`` status display; the tool dispatchers use
    :func:`_get_capability_backend`.
    """
...
def _get_capability_backend(capability: str) -> str:
    """Resolve the active backend NAME for a capability ("search" | "extract").

    Two stages, both single-path:

    1. **Explicit config passthrough.** ``web.{capability}_backend`` then
       ``web.backend`` from config is returned verbatim (lowercased). This
       mirrors the registry's "explicit config wins, ignoring availability"
       contract — a configured-but-uncredentialed backend surfaces a precise
       error from the provider instead of a silent switch. A search-only
       backend configured for ``extract`` is turned into a typed
       "search-only" error by :func:`web_extract_tool`, so we do NOT
       second-guess explicit config here.

    2. **Registry auto-detect.** With no explicit config, delegate to
       :func:`agent.web_search_registry.get_active_{capability}_provider`
       (single-eligible + availability-filtered legacy walk) and return its
       name, or ``""``.

    This collapses the former three-way fork (``_get_backend`` env-auto-detect
    / registry ``_resolve`` / ``_wsp_get_provider`` fallback) into one
    resolution path: the registry.
    """
    cfg = _load_web_config()
    explicit = (cfg.get(f"{capability}_backend") or "").lower().strip()
    if not explicit:
        explicit = (cfg.get("backend") or "").lower().strip()
    if explicit:
        return explicit
...
Create a markdown summary that captures all key information in a well-organized, scannable format. Include important quotes and code snippets in their original formatting. Focus on actionable information, specific details, and unique insights."""
    ...
    for attempt in range(max_retries):
        try:
            ...
            # NOTE: assign to `extracted`, NOT `content` — `content` is the
            # function parameter (the text to summarize). Shadowing it meant a
            # later `return content` would return the (empty) LLM output
            # instead of preserving the original input for the caller's fallback.
            extracted = extract_content_or_reasoning(response)
            if extracted:
                return extracted
            ...
            if attempt < max_retries - 1:
                ...
                continue
            return extracted  # Return whatever we got after exhausting retries
        except RuntimeError:
            logger.warning("No auxiliary model available for web content processing")
            return None
...
def _ensure_web_plugins_loaded() -> None:
    ...
    """
    try:
        from harness_cli.plugins import _ensure_plugins_discovered
