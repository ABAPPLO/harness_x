...
def _fire_approval_hook(hook_name: str, **kwargs) -> None:
    ...
    """
    try:
        from harness_cli.plugins import invoke_hook
    except Exception:
...
# theater. Mirrors _HARNESS_ENV_PATH; matches the HERMES_HOME override form as
# well as ~/.hermes/.
...
_SENSITIVE_WRITE_TARGET = (
    ...
    rf'{_HARNESS_ENV_PATH}|'
    rf'{_HARNESS_CONFIG_PATH}|'
    rf'{_SHELL_RC_FILES}|'
...
DANGEROUS_PATTERNS = [
    ...
    (rf'\bsed\s+-[^\s]*i.*(?:{_HARNESS_CONFIG_PATH}|{_HARNESS_ENV_PATH})', "in-place edit of Hermes config/env"),
    (rf'\bsed\s+--in-place\b.*(?:{_HARNESS_CONFIG_PATH}|{_HARNESS_ENV_PATH})', "in-place edit of Hermes config/env (long flag)"),
    ...
    (rf'\b(?:perl|ruby)\b.*(?:^|\s)-[^\s]*i\b.*(?:{_HARNESS_CONFIG_PATH}|{_HARNESS_ENV_PATH})', "in-place edit of Hermes config/env (perl/ruby)"),
...
def save_permanent_allowlist(patterns: set):
    ...
    try:
        from harness_cli.config import load_config, save_config
        config = load_config()
