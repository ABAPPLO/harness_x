...
def _is_hermes_provider_credential(name: str) -> bool:
    ...
    token, or similar) per ``_HARNESS_PROVIDER_ENV_BLOCKLIST``.
    ...
    """
    try:
        from tools.environments.local import _HARNESS_PROVIDER_ENV_BLOCKLIST
    except Exception:
        return False
    return name in _HARNESS_PROVIDER_ENV_BLOCKLIST
...
def register_env_passthrough(var_names: Iterable[str]) -> None:
    ...
    ``_HARNESS_PROVIDER_ENV_BLOCKLIST``) are rejected here to preserve
    ...
    for name in var_names:
        ...
        if _is_hermes_provider_credential(name):
            logger.warning(
                ...
                "credential %r (blocked by _HARNESS_PROVIDER_ENV_BLOCKLIST). "
...
def _load_config_passthrough() -> frozenset[str]:
    ...
    result: set[str] = set()
    try:
        from harness_cli.config import read_raw_config
        cfg = read_raw_config()
            ...
            for item in passthrough:
                ...
                if _is_hermes_provider_credential(name):
                    logger.warning(
                        ...
                        "by _HARNESS_PROVIDER_ENV_BLOCKLIST). Operator "
