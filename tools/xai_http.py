...
def has_xai_credentials() -> bool:
    ...
    try:
        ...
        auth_path = get_harness_home() / "auth.json"
...
def get_env_value(name: str, default=None):
    ...
    Wraps :func:`harness_cli.config.get_env_value` so tests can patch
    ``tools.xai_http.get_env_value`` to inject dotenv-only secrets into the
    xAI credential resolver.
    """
    try:
        from harness_cli.config import get_env_value as _hermes_get_env_value
...
def harness_xai_user_agent() -> str:
...
def resolve_xai_http_credentials(*, force_refresh: bool = False) -> Dict[str, str]:
    ...
    to ``XAI_API_KEY`` resolved via ``harness_cli.config.get_env_value`` so keys
