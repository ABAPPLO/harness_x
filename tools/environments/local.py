...
from harness_cli._subprocess_compat import windows_hide_flags
...
_HARNESS_PROVIDER_ENV_FORCE_PREFIX = "_HERMES_FORCE_"
...
def _build_provider_env_blocklist() -> frozenset:
    ...
    try:
        from harness_cli.config import OPTIONAL_ENV_VARS
...
_HARNESS_PROVIDER_ENV_BLOCKLIST = _build_provider_env_blocklist()


def _inject_context_hermes_home(env: dict) -> None:
    """Bridge the context-local Hermes home override into subprocess env."""
    try:
        from harness_constants import get_harness_home_override

        value = get_harness_home_override()
        if value:
...
def _sanitize_subprocess_env(base_env: dict | None, extra_env: dict | None = None) -> dict:
    ...
    for key, value in (base_env or {}).items():
        if key.startswith(_HARNESS_PROVIDER_ENV_FORCE_PREFIX):
            continue
        if key not in _HARNESS_PROVIDER_ENV_BLOCKLIST or _is_passthrough(key):
            sanitized[key] = value

    for key, value in (extra_env or {}).items():
        if key.startswith(_HARNESS_PROVIDER_ENV_FORCE_PREFIX):
            real_key = key[len(_HARNESS_PROVIDER_ENV_FORCE_PREFIX):]
            sanitized[real_key] = value
        elif key not in _HARNESS_PROVIDER_ENV_BLOCKLIST or _is_passthrough(key):
            sanitized[key] = value

    _inject_context_hermes_home(sanitized)

    # Per-profile HOME isolation for background processes (same as _make_run_env).
    from harness_constants import get_subprocess_home
...
def _find_bash() -> str:
    ...
    raise RuntimeError(
        "Git Bash not found. Harness Agent requires Git for Windows on Windows.\n"
...
def _make_run_env(env: dict) -> dict:
    ...
    for k, v in merged.items():
        if k.startswith(_HARNESS_PROVIDER_ENV_FORCE_PREFIX):
            real_key = k[len(_HARNESS_PROVIDER_ENV_FORCE_PREFIX):]
            run_env[real_key] = v
        elif k not in _HARNESS_PROVIDER_ENV_BLOCKLIST or _is_passthrough(k):
            run_env[k] = v
        ...
        **Windows:** hardcoded ``/tmp`` is wrong in two ways — native Python
        ...
        if _IS_WINDOWS:
            ...
            try:
                ...
                cache_dir = get_harness_home() / "cache" / "terminal"
            except Exception:
