...
# ---------------------------------------------------------------------------
# HarnessMCPOAuthProvider — OAuthClientProvider subclass with disk-watch
# ---------------------------------------------------------------------------
...
def _make_hermes_provider_class() -> Optional[type]:
    ...
    class HarnessMCPOAuthProvider(OAuthClientProvider):
        ...
        async def _initialize(self) -> None:
            ...
            Paired with :class:`HarnessTokenStorage` persisting an absolute
            ...
            from tools.mcp_oauth import HarnessTokenStorage
            if (
                isinstance(storage, HarnessTokenStorage)
            ...
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                for url in build_oauth_authorization_server_metadata_discovery_urls(
                    ...
                    if asm:
                        ...
                        storage = self.context.storage
                        from tools.mcp_oauth import HarnessTokenStorage
                        if isinstance(storage, HarnessTokenStorage):
        ...
        def _persist_oauth_metadata_if_changed(self) -> None:
            ...
            storage = self.context.storage
            from tools.mcp_oauth import HarnessTokenStorage
            if not isinstance(storage, HarnessTokenStorage):
                return
    ...
    return HarnessMCPOAuthProvider


# Cached at import time. Tested and used by :class:`MCPOAuthManager`.
_HARNESS_PROVIDER_CLS: Optional[type] = _make_hermes_provider_class()
...
class MCPOAuthManager:
    ...
    def _build_provider(
        ...
        Constructs :class:`HarnessMCPOAuthProvider` directly using the helpers
        ...
        Returns None if the MCP SDK's OAuth support is unavailable.
        """
        if _HARNESS_PROVIDER_CLS is None:
            logger.warning(
        ...
        storage = HarnessTokenStorage(server_name)
