"""Shared constants for Harness Agent.
...
def getenv(key: str, default: Optional[str] = None) -> Optional[str]:
    """Get env var with HARNESS_* priority over HERMES_*.

    For any key starting with 'HERMES_', also check the 'HARNESS_' equivalent.
    HARNESS_* always wins if set. This allows a clean migration path from
    hermes-agent to harness_x without breaking existing deployments.
    """
    if key.startswith("HERMES_"):
        harness_key = "HARNESS_" + key[7:]
        val = os.environ.get(harness_key)
        if val is not None:
            return val
...
_UNSET = object()
_HARNESS_HOME_OVERRIDE: ContextVar[str | object] = ContextVar(
    "_HARNESS_HOME_OVERRIDE", default=_UNSET
)


def set_harness_home_override(path: str | Path | None) -> Token:
    ...
    return _HARNESS_HOME_OVERRIDE.set(value)


def reset_harness_home_override(token: Token) -> None:
    """Restore the previous context-local Hermes home override."""
    _HARNESS_HOME_OVERRIDE.reset(token)


def get_harness_home_override() -> str | None:
    """Return the active context-local Hermes home override, if any."""
    override = _HARNESS_HOME_OVERRIDE.get()
    if override is _UNSET or not override:
        return None
    return str(override)


def _get_platform_default_harness_home() -> Path:
    """Return the platform-native default Hermes home path."""
    if sys.platform == "win32":
        local_appdata = os.environ.get("LOCALAPPDATA", "").strip()
        base = Path(local_appdata) if local_appdata else Path.home() / "AppData" / "Local"
        return base / "harness_x"
    return Path.home() / ".harness_x"
...
def get_harness_home() -> Path:
    ...
    """
    override = get_harness_home_override()
    if override:
    ...
    if not _profile_fallback_warned:
        try:
            fallback_home = _get_platform_default_harness_home()
    ...
    return _get_platform_default_harness_home()


def get_default_harness_root() -> Path:
    ...
    """
    native_home = _get_platform_default_harness_home()
...
def get_optional_skills_dir(default: Path | None = None) -> Path:
    ...
    if default is not None:
        return default
    return get_harness_home() / "optional-skills"
...
def get_optional_mcps_dir(default: Path | None = None) -> Path:
    ...
    if default is not None:
        return default
    return get_harness_home() / "optional-mcps"
...
def get_bundled_skills_dir(default: Path | None = None) -> Path:
    ...
    if default is not None:
        return default
    return get_harness_home() / "skills"


def get_harness_dir(new_subpath: str, old_name: str) -> Path:
...
def get_subprocess_home() -> str | None:
    ...
    **never** modified — only subprocess environments should inject this value.
    ...
    """
    hermes_home = get_harness_home_override() or os.getenv("HERMES_HOME")
    if not hermes_home:
        return None
...
def get_config_path() -> Path:
    ...
    Replaces the ``get_harness_home() / "config.yaml"`` pattern repeated
    in 7+ files (skill_utils.py, hermes_logging.py, hermes_time.py, etc.).
    """
    return get_harness_home() / "config.yaml"
...
def apply_ipv4_preference(force: bool = False) -> None:
    ...
    if getattr(socket.getaddrinfo, "_harness_ipv4_patched", False):
        return
    ...
    _ipv4_getaddrinfo._harness_ipv4_patched = True  # type: ignore[attr-defined]
