...
from harness_cli.config import get_harness_home
from harness_constants import OPENROUTER_BASE_URL
...
def _apply_user_default_headers(headers: dict | None) -> dict | None:
    ...
    """
    try:
        from harness_cli.config import cfg_get, load_config
...
# in lockstep with harness_cli.__version__ across every Portal call site
...
def _nous_extra_body() -> dict:
    ...
    Computed at call time so a hot-reloaded ``harness_cli.__version__`` is
...
_AUTH_JSON_PATH = get_harness_home() / "auth.json"
...
def _codex_cloudflare_headers(access_token: str) -> Dict[str, str]:
    ...
    """
    headers = {
        "User-Agent": "codex_cli_rs/0.0.0 (Harness Agent)",
...
def _endpoint_speaks_anthropic_messages(base_url: str) -> bool:
    ...
    Mirrors ``harness_cli.runtime_provider._detect_api_mode_for_url`` so the
...
def _nous_api_key(provider: dict) -> str:
    ...
    from harness_cli.auth import _nous_invoke_jwt_is_usable
...
def _resolve_nous_runtime_api(*, force_refresh: bool = False) -> Optional[tuple[str, str]]:
    ...
    """
    try:
        from harness_cli.auth import resolve_nous_runtime_credentials
...
def _resolve_xai_oauth_for_aux() -> Optional[Tuple[str, str]]:
    ...
    Falls back to ``harness_cli.auth``'s singleton runtime resolver for older
    auth-store-only logins. Returns ``None`` if the user is not authenticated
    with xAI Grok OAuth.
    """
    try:
        from harness_cli.auth import (
    ...
    try:
        from harness_cli.auth import resolve_xai_oauth_runtime_credentials
...
def _read_codex_access_token() -> Optional[str]:
    ...
    try:
        from harness_cli.auth import _read_codex_tokens
...
def _resolve_api_key_provider() -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    """
    try:
        from harness_cli.auth import PROVIDER_REGISTRY, resolve_api_key_provider_credentials
    except ImportError:
    ...
    for provider_id, pconfig in PROVIDER_REGISTRY.items():
        ...
        if provider_id == "anthropic":
            ...
            try:
                from harness_cli.auth import is_provider_explicitly_configured
        ...
        if pool_present:
            ...
            if base_url_host_matches(base_url, "api.kimi.com"):
                ...
                from harness_cli.models import copilot_default_headers
...
def _try_nous(vision: bool = False) -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    model = _NOUS_MODEL
    try:
        from harness_cli.models import get_nous_recommended_aux_model
...
def _resolve_custom_runtime() -> Tuple[Optional[str], Optional[str], Optional[str]]:
    ...
    """
    try:
        from harness_cli.runtime_provider import resolve_runtime_provider
...
def _try_azure_foundry(
    *,
    ...
    :func:`harness_cli.runtime_provider._resolve_azure_foundry_runtime` —
    ...
    * Non-default ``model.base_url`` overrides are honored.
    ...
    """
    try:
        from harness_cli.runtime_provider import _resolve_azure_foundry_runtime
        from harness_cli.auth import AuthError
        from harness_cli.config import load_config
    except ImportError:
        return None, None
...
def _nous_portal_account_has_fresh_paid_access() -> bool:
    ...
    try:
        from harness_cli.nous_account import get_nous_portal_account_info
...
def _recoverable_pool_provider(
    ...
    if main_runtime:
        ...
        if rt_provider and rt_provider not in {"", "auto", "custom"}:
            try:
                from harness_cli.auth import PROVIDER_REGISTRY
...
def _refresh_provider_credentials(provider: str) -> bool:
    ...
    try:
        if normalized == "openai-codex":
            from harness_cli.auth import resolve_codex_runtime_credentials
...
def _to_async_client(sync_client, model: str, is_vision: bool = False):
    ...
    if base_url_host_matches(sync_base_url, "openrouter.ai"):
        ...
        from harness_cli.copilot_auth import copilot_request_headers
...
def _normalize_resolved_model(model_name: Optional[str], provider: str) -> Optional[str]:
    ...
    if not model_name:
        return model_name
    try:
        from harness_cli.model_normalize import normalize_model_for_provider
...
def resolve_provider_client(
    ...
    try:
        from harness_cli.runtime_provider import _get_named_custom_provider
    ...
    try:
        ...
        logger.debug("harness_cli.auth not available for provider %s", provider)
        return None, None
    ...
    if pconfig.auth_type == "api_key":
        ...
        if provider == "copilot" and final_model and not raw_codex:
            try:
                from harness_cli.models import _should_use_copilot_responses_api
...
def _get_auxiliary_task_config(task: str) -> Dict[str, Any]:
    ...
    :meth:`harness_cli.plugins.PluginContext.register_auxiliary_task`) the
    ...
    try:
        from harness_cli.plugins import get_plugin_auxiliary_tasks
