...
from harness_constants import get_harness_home, get_skills_dir, is_wsl
...
_HARNESS_MD_NAMES = (".hermes.md", "HERMES.md")
...
DEFAULT_AGENT_IDENTITY = (
    "You are Harness Agent, an intelligent AI assistant created by Nous Research. "
...
HERMES_AGENT_HELP_GUIDANCE = (
    "You run on Harness Agent (by Nous Research). When the user needs help with "
...
def _skills_prompt_snapshot_path() -> Path:
    return get_harness_home() / ".skills_prompt_snapshot.json"
...
def build_skills_system_prompt(
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "or troubleshoot Harness Agent itself — its CLI, config, models, providers, tools, "
...
def build_nous_subscription_prompt(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    try:
        from harness_cli.nous_subscription import get_nous_subscription_features
...
def load_soul_md() -> Optional[str]:
    ...
    """
    try:
        from harness_cli.config import ensure_hermes_home
        ensure_hermes_home()
    except Exception as e:
        logger.debug("Could not ensure HERMES_HOME before loading SOUL.md: %s", e)

    soul_path = get_harness_home() / "SOUL.md"
