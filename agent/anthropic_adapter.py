"""Anthropic Messages API adapter for Harness Agent.
...
_HARNESS_OAUTH_FILE = get_harness_home() / ".anthropic_oauth.json"
...
def run_hermes_oauth_login_pure() -> Optional[Dict[str, Any]]:
    ...
    try:
        from harness_cli.auth import _can_open_graphical_browser as _can_open_gui
    except Exception:
...
def read_hermes_oauth_credentials() -> Optional[Dict[str, Any]]:
    ...
    if _HARNESS_OAUTH_FILE.exists():
        try:
            data = json.loads(_HARNESS_OAUTH_FILE.read_text(encoding="utf-8"))
    ...
    if is_oauth:
        ...
        for block in system:
            if isinstance(block, dict) and block.get("type") == "text":
                text = block.get("text", "")
                text = text.replace("Harness Agent", "Claude Code")
