...
* Python plugins and shell hooks compose naturally: both flow through
  :func:`harness_cli.plugins.invoke_hook` and its aggregators.  Python
...
def register_from_config(
    ...
    *,
    ...
    ``cfg`` is the full parsed config dict (``harness_cli.config.load_config``
    ...
    from harness_cli.plugins import get_plugin_manager
...
def _parse_hooks_block(hooks_cfg: Any) -> List[ShellHookSpec]:
    ...
    """
    from harness_cli.plugins import VALID_HOOKS
...
def _parse_response(event: str, stdout: str) -> Optional[Dict[str, Any]]:
    ...
    :func:`harness_cli.plugins.get_pre_tool_call_block_message`.  This is
...
def allowlist_path() -> Path:
    ...
    return get_harness_home() / ALLOWLIST_FILENAME
...
def run_once(
    ...
    ``kwargs`` is the same dict that :func:`harness_cli.plugins.invoke_hook`
