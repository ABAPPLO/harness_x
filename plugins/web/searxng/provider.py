...
class SearXNGWebSearchProvider(WebSearchProvider):
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        # `score` can be null or a non-numeric string for engines that can't
        # score results — `r.get("score", 0)` still returns None when the key
        # is present-but-null, and float(None) would crash the whole search.
        def _safe_score(r):
            try:
