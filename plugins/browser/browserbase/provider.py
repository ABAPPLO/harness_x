...
def _env(key: str, default: str | None = None) -> str | None:
    """Config-aware env read — honors ``hermes config set`` / ~/.harness_x/.env,
    falling back to raw process env. Drop-in for ``os.environ.get`` so
...
class BrowserbaseBrowserProvider(BrowserProvider):
    ...
    def _get_config_or_none(self) -> Optional[Dict[str, Any]]:
        api_key = _env("BROWSERBASE_API_KEY")
        project_id = _env("BROWSERBASE_PROJECT_ID")
    ...
    def create_session(self, task_id: str) -> Dict[str, object]:
        ...
        enable_proxies = _env("BROWSERBASE_PROXIES", "true").lower() != "false"
        enable_advanced_stealth = (
            _env("BROWSERBASE_ADVANCED_STEALTH", "false").lower() == "true"
        )
        enable_keep_alive = (
            _env("BROWSERBASE_KEEP_ALIVE", "true").lower() != "false"
        )
        custom_timeout_ms = _env("BROWSERBASE_SESSION_TIMEOUT")
