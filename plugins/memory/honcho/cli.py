...
def cmd_sync(args) -> None:
    ...
    """
    try:
        from harness_cli.profiles import list_profiles
    ...
    created = 0
    skipped = 0
    # list_profiles() returns profile names as List[str] (single-profile stub in
    # harness_x), so iterate the string directly — not p.name.
    for p in profiles:
        if p == "default":
            continue
        if clone_honcho_for_profile(p):
            print(f"  + {p} -> {profile_host_key(p)}")
            created += 1
        else:
            skipped += 1
...
def _local_config_path() -> Path:
    ...
    """
    return get_harness_home() / "honcho.json"
...
def _all_profile_host_configs() -> list[tuple[str, str, dict]]:
    ...
    for p in profiles:
        if p == "default":
            continue
        h = f"{HOST}.{p}"
        results.append((p, h, hosts.get(h, {})))
...
def honcho_command(args) -> None:
    ...
    if sub == "setup":
        ...
        from harness_cli.memory_setup import cmd_setup_provider
