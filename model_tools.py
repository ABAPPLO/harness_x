...
try:
    from harness_cli.plugins import discover_plugins
    discover_plugins()
...
def get_tool_definitions(
    ...
    if quiet_mode:
        try:
            from harness_cli.config import get_config_path
...
def _compute_tool_definitions(
    ...
    # window). Core Hermes tools (toolsets._HARNESS_CORE_TOOLS) are NEVER
...
def _resolve_active_context_length() -> int:
    ...
    """
    try:
        from harness_cli.config import load_config as _load
        cfg = _load() or {}
...
def _emit_post_tool_call_hook(
    *,
    ...
    """
    try:
        from harness_cli.plugins import has_hook, invoke_hook
