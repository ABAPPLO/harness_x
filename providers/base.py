...
def _profile_user_agent() -> str:
    ...
    """
    try:
        from harness_cli import __version__ as _ver  # lazy: avoid layer cycle at import time
...
class ProviderProfile:
    ...
    def fetch_models(
        ...
        *,
        ...
        import httpx  # core dep; lazy import keeps it off profile-system startup

        headers = {"Accept": "application/json"}
        # Some providers (e.g. OpenCode Zen) sit behind a WAF that blocks
        # the default Python User-Agent. Send a generic hermes-cli UA so the
        # catalog endpoint is reachable.
        ...
        # Request phase and parse phase are handled separately so a
        # transport-misconfiguration ValueError (e.g. an ``ALL_PROXY`` set to
        # an unsupported ``socks://`` scheme — httpx raises a plain ValueError
        # for it) is reported as "couldn't transact" (debug) instead of being
        # misreported as a response-parse failure. Only a genuine non-JSON 2xx
        # body reaches the parse phase and warns about a likely wrong base_url.
        try:
            # follow_redirects + raise_for_status preserve urllib semantics:
            # GET follows redirects and a non-2xx is a failure (not a parse
            # attempt on an error body).
            with httpx.Client(timeout=timeout, follow_redirects=True) as client:
                resp = client.get(url, headers=headers)
                resp.raise_for_status()
        except httpx.HTTPError as exc:
            # Transport failure OR non-2xx status (HTTPStatusError subclasses
            # HTTPError). Common (offline, provider down, wrong URL) and an
            # expected fallback path; keep at debug so it doesn't spam.
        ...
        # 200), so warn loudly instead of silently falling back.
        try:
            data = resp.json()
            ...
            logger.warning(
                "fetch_models(%s): could not parse /models response from %r "
                "(is base_url correct?): %s",
                self.name, url, exc,
            )
            return None
        except Exception as exc:  # noqa: BLE001 — defensive net; contract is "return None, never raise"
            logger.warning("fetch_models(%s): unexpected error: %s", self.name, exc)
            return None
