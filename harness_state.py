#!/usr/bin/env python3
"""
SQLite State Store for Harness Agent.
...
DEFAULT_DB_PATH = get_harness_home() / "state.db"
...
def apply_wal_with_fallback(
    ...
    *,
    ...
    Shared by :class:`SessionDB` and ``harness_cli.kanban_db.connect`` so
...
class SessionDB:
    ...
    def _sqlite_supports_fts5(self, cursor: sqlite3.Cursor) -> bool:
        try:
            cursor.execute("CREATE VIRTUAL TABLE temp._harness_fts5_probe USING fts5(x)")
            cursor.execute("DROP TABLE temp._harness_fts5_probe")
            return True
        except sqlite3.OperationalError as exc:
