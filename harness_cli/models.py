...
from harness_cli import __version__ as _HARNESS_VERSION

# Identify ourselves so endpoints fronted by Cloudflare's Browser Integrity
# Check (error 1010) don't reject the default ``Python-urllib/*`` signature.
_HARNESS_USER_AGENT = f"hermes-cli/{_HARNESS_VERSION}"
...
def _codex_curated_models() -> list[str]:
    ...
    """
    from harness_cli.codex_models import DEFAULT_CODEX_MODELS, _add_forward_compat_models
...
def _nous_recommended_disk_path() -> "Path":
    ...
    return get_harness_home() / "cache" / "nous_recommended_cache.json"
...
def fetch_openrouter_models(
    ...
    *,
    ...
    try:
        from harness_cli.model_catalog import get_curated_openrouter_models
...
def get_curated_nous_model_ids() -> list[str]:
    ...
    """
    try:
        from harness_cli.model_catalog import get_curated_nous_models
...
def fetch_models_with_pricing(
    ...
    *,
    ...
    headers: dict[str, str] = {
        ...
        "User-Agent": _HARNESS_USER_AGENT,
    }
    if api_key:
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    for pid in provider_order:
        ...
        has_creds = False
        try:
            from harness_cli.auth import get_auth_status, has_usable_secret
            if pid == "custom":
...
def _resolve_static_model_alias(
    ...
    try:
        from harness_cli.model_switch import MODEL_ALIASES
    except Exception:
        return None
...
def normalize_provider(provider: Optional[str]) -> str:
    ...
    ``harness_cli.auth.resolve_provider()`` to resolve it to a concrete
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    """
    try:
        from harness_cli.auth import resolve_api_key_provider_credentials
    ...
    try:
        from harness_cli.auth import read_credential_pool
        from harness_cli.copilot_auth import (
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    if normalized == "openai-codex":
        from harness_cli.codex_models import get_codex_model_ids
    ...
    if normalized == "nous":
        ...
        try:
            from harness_cli.auth import fetch_nous_models, resolve_nous_runtime_credentials
            creds = resolve_nous_runtime_credentials()
            if creds:
...
def _provider_models_cache_path() -> Path:
    from harness_constants import get_harness_home
    return get_harness_home() / "provider_models_cache.json"
...
def _lmstudio_request_headers(api_key: Optional[str] = None) -> dict:
    ...
    headers = {"User-Agent": _HARNESS_USER_AGENT}
...
def probe_api_models(
    ...
    tried: list[str] = []
    headers: dict[str, str] = {"User-Agent": _HARNESS_USER_AGENT}
...
def _ollama_cloud_cache_path() -> Path:
    ...
    from harness_constants import get_harness_home
    return get_harness_home() / "ollama_cloud_models_cache.json"
