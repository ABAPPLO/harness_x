"""
Multi-provider authentication system for Harness Agent.
...
from harness_cli.config import get_harness_home, get_config_path, read_raw_config
from harness_constants import OPENROUTER_BASE_URL, secure_parent_dir
...
def _resolve_api_key_provider_secret(
    ...
    if provider_id == "copilot":
        ...
        try:
            from harness_cli.copilot_auth import resolve_copilot_token, get_copilot_api_token
...
def _format_nous_entitlement_auth_error(error: AuthError) -> str:
    try:
        from harness_cli.nous_account import (
...
def _auth_file_path() -> Path:
    path = get_harness_home() / "auth.json"
...
def _global_auth_file_path() -> Optional[Path]:
    ...
    """
    try:
        from harness_constants import get_default_harness_root
        global_root = get_default_harness_root()
    except Exception:
        return None
    profile_home = get_harness_home()
    try:
...
def _load_global_auth_store() -> Dict[str, Any]:
    ...
    ``get_default_harness_root()`` for a profile-shaped HERMES_HOME can
...
def _get_config_hint_for_unknown_provider(provider_name: str) -> str:
    ...
    """
    try:
        from harness_cli.config import validate_config_structure
...
def _spotify_interactive_setup(redirect_uri_hint: str) -> str:
    ...
    """
    from harness_cli.config import save_env_value
...
# ``get_default_harness_root()`` returns — ``~/.hermes`` on Linux/macOS,
...
def _nous_shared_auth_dir() -> Path:
    ...
    :func:`hermes_constants.get_default_harness_root` returns — so
    ...
    from harness_constants import get_default_harness_root
    return get_default_harness_root() / "shared"
...
def _nous_shared_store_path() -> Path:
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        from harness_constants import get_default_harness_root
        real_home_shared = (
            get_default_harness_root() / "shared" / NOUS_SHARED_STORE_FILENAME
...
def _get_azure_foundry_auth_status() -> Dict[str, Any]:
    ...
    try:
        from harness_cli.config import load_config, get_env_value
        cfg = load_config()
    except Exception:
        cfg = {}
...
def _prompt_model_selection(
    ...
    """
    from harness_cli.models import _format_price_per_mtok
    ...
    try:
        from harness_cli.curses_ui import curses_radiolist
...
def _save_model_choice(model_id: str) -> None:
    ...
    """
    from harness_cli.config import save_config, load_config
...
def _login_openai_codex(
    ...
    *,
    ...
    print()
    print("Login successful!")
    from harness_constants import display_harness_home as _dhh
...
def _login_nous(args, pconfig: ProviderConfig) -> None:
    ...
    try:
        ...
        try:
            ...
            from harness_cli.models import (
