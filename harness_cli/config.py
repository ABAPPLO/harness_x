...
from harness_cli.secret_prompt import masked_secret_prompt
...
from harness_cli.colors import Colors, color
from harness_cli.default_soul import DEFAULT_SOUL_MD
...
def get_managed_system() -> Optional[str]:
    ...
    managed_marker = get_harness_home() / ".managed"
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    ...
    """
    stamp = get_harness_home() / ".install_method"
    try:
...
Harness Agent runs as a published image (nousresearch/hermes-agent), not a
...
def get_container_exec_info() -> Optional[dict]:
    ...
    from harness_constants import is_container
    if is_container():
        return None

    container_mode_file = get_harness_home() / ".container-mode"
...
# Re-export from harness_constants — canonical definition lives there.
from harness_constants import get_harness_home  # noqa: F811,E402
...
DEFAULT_CONFIG = {
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *Harness Agent*" header.
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``harness_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 21:
        ...
        if "enabled" not in plugins_cfg:
            ...
            try:
                user_plugins_dir = get_harness_home() / "plugins"
    ...
    if current_ver < 23:
        try:
            curator_dir = get_harness_home() / "logs" / "curator"
...
def show_config():
    ...
    from harness_cli.auth import get_anthropic_key
